...
export async function updateUser(idParam, patch) {
  ...
  // Deixa tryUpdateRoutes unificar id/_id
  const body = { ...patch };
...
/* ============== Upload de avatar (robusto) ============== */
export async function uploadAvatar(userId, file) {
  // Tenta variações de endpoint e nome do campo (file/avatar/image/photo)
  const endpoints = [
    { path: `/users/${userId}/avatar`, params: undefined },
    { path: `/upload/avatar`,          params: { userId } },
  ];
  const fieldNames = ["file", "avatar", "image", "photo"];

  const postAvatar = async (path, fieldName, params) => {
    const fd = new FormData();
    fd.append(fieldName, file);
    return api(path, { method: "POST", body: fd, params });
  };

  for (const ep of endpoints) {
    for (const fn of fieldNames) {
      try {
        const r = await postAvatar(ep.path, fn, ep.params);
...
/* ============== Remover avatar do servidor (com fallbacks) ============== */
export async function deleteAvatar(userId) {
  // 1) Rotas prováveis do backend
  const tries = [
    { method: "DELETE", path: `/users/${userId}/avatar` },
    { method: "POST",   path: `/users/${userId}/avatar`, body: { remove: true } },
    { method: "DELETE", path: `/upload/avatar`,           params: { userId } },
    { method: "POST",   path: `/upload/avatar/clear`,     body: { userId } },
    // fallback hard: apagar campos no user
    { method: "PATCH",  path: `/users/${userId}`,         body: { avatarUrl: "", photoUrl: "", imageUrl: "" } },
  ];
